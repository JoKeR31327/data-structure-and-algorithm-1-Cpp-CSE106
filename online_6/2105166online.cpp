...
int calcMinMoves(int rows, int cols, const vector<Rider *> &riders,vector<vector<int>> &deathSquares) {
    int minMove = INT_MAX;
    ...
    for (size_t i = 0; i < rows; i++) {
        for (size_t j = 0; j < cols; j++) {
            ...
            for (const auto rider : riders) {
                rider->board[i][j] =deathSquares[i][j];
...
int main() {
    ...
    cin >> d;
    vector<vector<int>> deathSquares(rows, vector<int>(cols, 0));
    for (int i = 0; i < d; i++) {
        cin >> x >> y;
        deathSquares[x][y] = -1;
    }
    int minMove = calcMinMoves(rows, cols, riders,deathSquares);
